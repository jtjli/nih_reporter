...
def process(f, doMerge = True):
    ...
    # Convert long columns to int64
    for cn in getFieldsByType(projects_schema,'LongType()'):
       csv2[cn] = csv2[cn].astype(int64)

    # Convert date columns to DateType
    for cn in getFieldsByType(projects_schema,'DateType()'):
        assert max(csv2[cn].map(lambda x: len(str(x)))) <= 10, f"date format not expected in {f}"
...
csv2 = process(fs[23], False)

csv2.dtypes
csv2["APPLICATION_TYPE"].unique()
...
csv.dtypes
max(csv["PROJECT_START"].map(lambda x: len(str(x))))
...
debug1 = update_pd_df.iloc[0:1,]
debug1.dtypes
debug1["APPLICATION_TYPE"] = debug1["APPLICATION_TYPE"].astype(int64)
#debug1["APPLICATION_TYPE"].astype(int64)
debug1.dtypes

debug1.loc[0,"APPLICATION_TYPE"] = 5
display(debug1.iloc[:,:])
updatedf = spark.createDataFrame(debug1,schema)

update_pd_df.iloc[0:1,]
